...
function Restart-AllDeadJobs {
    ...
    if (-not (Confirm-Action "Bude odblokovano $($dead.Count) jobu pro dalsi naplanovani. Tato akce muze pustit mnoho jobu najednou." -Required "RESTART")) { Write-Host "Zruseno."; return }
    foreach ($job in $dead) {
        try {
...
function Export-AllFailedJobLogs {
    ...
    $failedJobs = @($status.pool.jobs | Where-Object {
        $_.status -in @("failed_final", "failed_restart")
    } | Sort-Object status, job_name)

    if ($failedJobs.Count -eq 0) {
        Write-Host "Nejsou nalezeny zadne failed_final ani failed_restart joby." -ForegroundColor Green
        return
    }

    Write-Host "Nalezeno failed jobu: $($failedJobs.Count)" -ForegroundColor Yellow
    $failedJobs | ForEach-Object { Write-JobLine $_ }
    Write-Host ""

    if (-not (Ensure-Token)) {
        Write-Host "Export zrusen - bez tokenu nelze nacist job logy." -ForegroundColor Yellow
        return
    }

    $bytesRaw = Read-Host "Kolik bytu z konce KAZDEHO logu, prazdne = 16384"
    $bytes = 16384
    if (-not [string]::IsNullOrWhiteSpace($bytesRaw)) {
        if (-not [int]::TryParse($bytesRaw, [ref]$bytes) -or $bytes -le 0) {
            Write-Host "Neplatna hodnota. Pouzivam 16384 bytu." -ForegroundColor Yellow
            $bytes = 16384
        }
    }

    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $defaultDir = Join-Path (Get-Location) "oflow_failed_logs_$stamp"
    $dirInput = Read-Host "Cilova slozka, prazdne = $defaultDir"
    $outputDir = if ([string]::IsNullOrWhiteSpace($dirInput)) { $defaultDir } else { $dirInput.Trim() }
    ...
    try {
        ...
        Write-Host "Nelze vytvorit cilovou slozku: $($_.Exception.Message)" -ForegroundColor Red
        return
    }

    $combinedFile = Join-Path $outputDir "ALL_FAILED_LOGS_$stamp.txt"
    $summaryCsv   = Join-Path $outputDir "failed_jobs_summary_$stamp.csv"
    $summaryTxt   = Join-Path $outputDir "export_summary_$stamp.txt"
    ...
    $combined.Add("OFLOW FAILED JOB LOG EXPORT")
    $combined.Add("Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
    $combined.Add("BaseUrl: $script:BaseUrl")
    $combined.Add("Requested bytes per log: $bytes")
    $combined.Add("Failed jobs: $($failedJobs.Count)")
    $combined.Add("")

    foreach ($job in $failedJobs) {
        $safeName = ($job.job_name -replace '[\\/:*?\"<>|]', '_')
        $fileName = "{0}_{1}_{2}.log" -f $job.status, $job.id, $safeName
        $filePath = Join-Path $outputDir $fileName

        Write-Host "Nacitam: $($job.job_name) [ID $($job.id), $($job.status)] ..." -NoNewline

        try {
            $log = Invoke-OflowRequest -Path "/job_log/$([uri]::EscapeDataString($job.job_name))?bytes=$bytes" -UseToken
            ...
            $header = @(
                ...
                "LAST CHANGE:    $($job.last_change)"
                "BYTES RETURNED: $($log.bytes_returned) / $($log.bytes_requested)"
                "FILE SIZE:      $($log.file_size)"
                "============================================================"
                ""
            )

            @($header + [string]$log.content) | Set-Content -Path $filePath -Encoding UTF8

            $combined.AddRange([string[]]$header)
            $combined.Add([string]$log.content)
            $combined.Add("")
            $combined.Add("")
            ...
            $summary.Add([PSCustomObject]@{
                ...
                LastChange    = $job.last_change
                ExportResult  = "OK"
                BytesReturned = $log.bytes_returned
            ...
            $errorText | Set-Content -Path $filePath -Encoding UTF8
            $combined.AddRange([string[]]$errorText)
            $combined.Add("")
    ...
    $combined | Set-Content -Path $combinedFile -Encoding UTF8
    $summary | Export-Csv -Path $summaryCsv -NoTypeInformation -Encoding UTF8

    @(
        "OFLOW failed logs export summary"
        "Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
        "BaseUrl: $script:BaseUrl"
        "Failed jobs found: $($failedJobs.Count)"
        "Successfully exported: $okCount"
        "Export errors: $failCount"
        "Bytes requested per job: $bytes"
        "Output directory: $outputDir"
        "Combined log: $combinedFile"
        "CSV summary: $summaryCsv"
    ) | Set-Content -Path $summaryTxt -Encoding UTF8

    Write-Host ""
    Write-Host "Export dokoncen." -ForegroundColor Green
    Write-Host "  Uspesne logy: $okCount"
    Write-Host "  Chyby exportu: $failCount"
    Write-Host "  Slozka: $outputDir"
    Write-Host "  Spolecny log: $combinedFile"
    Write-Host "  CSV prehled: $summaryCsv"
    Write-Host ""
    Write-Host "Poznamka: endpoint /job_log vraci poslednich N bytu logu, ne nutne cely soubor." -ForegroundColor Yellow
}
...
function Show-Menu {
    ...
    Write-Host ("10 - Job log jednoho jobu".PadRight($colWidth)) -NoNewline
    Write-Host ""

    Write-Host ("23 - Export logu vsech failed jobu".PadRight($colWidth)) -NoNewline
    Write-Host ""
