-- Retence BigQuery - MVP DDL (VODWH / VP_STG scope)
-- Scope: konfigurace pravidel + audit runu + audit tasku + stavovy model.
-- Target project: o2czvd1
-- Zdroj logiky: prevzato z EDW/sql/retention_ddl.sql, zmenen jen project_id.

-- =========================================================
-- 1) Konfiguracni tabulka pravidel
-- =========================================================
CREATE TABLE IF NOT EXISTS `o2czvd1.opr_data.table_retention` (
  ...
  is_active BOOL NOT NULL OPTIONS(description="Příznak aktivace retenčního pravidla pro BigQuery."),
...
OPTIONS (
  description = "Metadata retenčních pravidel pro centralizovaný maintenance proces v BQ (VODWH/VP_STG scope)"
);
...
-- =========================================================
CREATE TABLE IF NOT EXISTS `o2czvd1.opr_data.retention_run` (
...
-- =========================================================
CREATE TABLE IF NOT EXISTS `o2czvd1.opr_data.retention_task_run` (
...
-- =========================================================
CREATE TABLE IF NOT EXISTS `o2czvd1.opr_data.retention_status_model` (
...
MERGE `o2czvd1.opr_data.retention_status_model` T
USING (
...
-- =========================================================
CREATE OR REPLACE VIEW `o2czvd1.opr_data.v_retention_run_last_14d` AS
SELECT
  run_id,
  run_date,
  run_start_dttm,
  ...
  status,
  error_message
FROM `o2czvd1.opr_data.retention_run`
WHERE run_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
ORDER BY run_start_dttm DESC;

CREATE OR REPLACE VIEW `o2czvd1.opr_data.v_retention_task_failures_last_14d` AS
SELECT
  execution_date,
  run_id,
  retention_rule_id,
  ...
  started_dttm,
  finished_dttm
FROM `o2czvd1.opr_data.retention_task_run`
